...
SCHEDULE="57 20 * * *"  # 每日8:57 PM
TIMEZONE="Asia/Shanghai"

echo "🚀 正在设置 Cloud Scheduler 任务..."
echo "📅 执行时间: 每日晚上8:57 (北京时间)"
...
if gcloud scheduler jobs describe $JOB_NAME --location=$LOCATION --project=$PROJECT_ID > /dev/null 2>&1; then
    ...
    gcloud scheduler jobs update http $JOB_NAME \
        ...
        --project=$PROJECT_ID \
        --time-zone=$TIMEZONE \
        --description="Daily WeeklyReporter execution at 8:57 PM with RAMPUP and YueMeng partners"
...
echo ""
echo "🧪 测试建议:"
echo "  1. 手动触发测试: ./test_scheduler.sh"

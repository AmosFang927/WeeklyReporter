#!/bin/bash
# 部署修复后的ByteC Postback到Cloud Run
...
echo "🚀 开始部署更新后的ByteC Postback到Cloud Run"
echo "📋 项目: $PROJECT_ID"
...
gcloud run deploy $SERVICE_NAME \
    ...
    --timeout 300 \
    --concurrency 100 \
    --set-env-vars "DEBUG=false,LOG_LEVEL=INFO,DATABASE_URL=postgresql+asyncpg://postback_admin:ByteC2024PostBack_CloudSQL_20250708@34.124.206.16:5432/postback_db,DATABASE_ECHO=false,DATA_RETENTION_DAYS=30" \
...
# 测试数据库连接
echo "🔍 测试数据库连接..."
DB_TEST_RESPONSE=$(curl -s "$SERVICE_URL/involve/db-test" || echo "failed")
if [[ $DB_TEST_RESPONSE == *"db_connected"* ]]; then
    echo "✅ 数据库连接测试通过"
else
    echo "⚠️ 数据库连接测试失败"
fi
...
echo "curl $SERVICE_URL/involve/db-test"
